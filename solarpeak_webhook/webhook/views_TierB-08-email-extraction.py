...
def vapi_webhook(request):
    try:
        ...
        call_id = call_obj.get("id") or artifact.get("call", {}).get("id")
        ...
        # endedReason + durationSeconds are top-level payload in end-of-call-report
        ...
        # 1) ConfirmedEmail mapping by call_id
        ...
        if not email.endswith("@noemail.local"):
            ...
            assert "@" in email and "." in email.split("@")[-1], f"Invalid email detected: {email}"
            properties = {
                "email": email,
