...
def confirm_email_tool(request):
    ...
    call_id_from_payload = (
        payload.get("call", {}).get("id")
        or (message.get("call") or {}).get("id")
        or ""
    )
    logger.info(f"confirm_email_tool call_id_from_payload: {call_id_from_payload}")
    ...
    try:
        ...
        if call_id:
            ...
            if rec and rec.email:
                email = rec.email
                logger.info(f"Using confirmed email for call {call_id}: {email}")

        # ✅ Only fallback if NO confirmed email exists at all
        if email is None:
            email = extract_email_from_transcript(transcript)
            if email:
                logger.warning(f"Using fallback transcript email for call {call_id}: {email}")
